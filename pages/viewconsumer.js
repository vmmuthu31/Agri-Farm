...
const ViewData = () => {
  ...
  const scanTable = async () => {
    try {
      data = await ddbDocClient.send(new ScanCommand({ TableName: "Consumers" }));
  ...
  return (
    <div className="container mx-auto py-10 flex flex-col w-screen h-screen items-center">
      ...
      <p className="text-3xl">View Consumer Data</p>
                ...
                <tbody>
                  {tableData.map((item) => (
                    <tr className="border-b" key={item.id}>
                      ...
                      <td className={Styles.tableData}>{item.consumerName}</td>
                      <td className={Styles.tableData}>{item.consumerDetails}</td>
                      ...
                      <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap text-center">
                        <Link
                          href={{
                            pathname: "/updateconsumer",
                            query: {
                              id: item.id,
                              dateAdded: item.dateAdded,
                              consumerName: item.consumerName,
                              consumerDetails: item.consumerDetails,
